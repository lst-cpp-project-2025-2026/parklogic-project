...
CameraSystem::CameraSystem(std::shared_ptr<EventBus> bus) : eventBus(bus) {
 ...
 // --- New tracking logic implementation ---
  eventTokens.push_back(eventBus->subscribe<TrackingStatusEvent>([this](const TrackingStatusEvent& e) {
    ...
    if (!this->isTracking) {
        // Reset zoom to normal when tracking stops
        this->camera.zoom = 1.0f; 
        // Reset camera target to world center to avoid staying in empty space
        if (this->worldWidth > 0 && this->worldHeight > 0) {
        this->camera.target = { worldWidth / 2.0f, worldHeight / 2.0f };
        }   
      } else {
        // Apply automatic zoom when tracking starts
